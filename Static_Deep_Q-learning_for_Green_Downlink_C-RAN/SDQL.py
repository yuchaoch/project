...
########################################################################################################################
    def GridBSRSRP(self):
        ...
        m_BSCtnCdt=pd_DF.values[:,1:]
        del pd_DF
        gc.collect()
        pd_DF=pd.read_csv((self.__docHeader+'0GenAntena.csv').replace('p/CSV_','p/net/CSV_'))
        m_AntPatten = np.round(40 - pd_DF.values[1:,1:].astype(np.float32),3)
        del pd_DF
        gc.collect()
        m_BSCdt = np.array(m_BSCtnCdt[:,4:6])

        m_AreaBorder = np.zeros((2,m_BSCdt.shape[1]))
        m_AreaBorder[0] = np.round(np.min(m_BSCdt,axis=0)-200)
        m_AreaBorder[1] = np.round(np.max(m_BSCdt,axis=0)+200)
        m_GridCdt=None
        for iX in np.arange(m_AreaBorder[0,0], m_AreaBorder[1,0], self.__step):
            if 0==(np.abs(iX)%50) or 25==(np.abs(iX)%50) :
                print('%.0fs: %s [%s, iX Prog] = [%d/%d, %.3f, %.2fMB]' % ((time.time()-self.__t_start), sys._getframe().f_lineno, sys._getframe(0).f_code.co_name,
                                                                         iX, m_AreaBorder[1,0], iX/m_AreaBorder[1,0], sys.getsizeof(m_GridCdt)/1e6))
            for jY in np.arange(m_AreaBorder[0,1], m_AreaBorder[1,1], self.__step):
                if m_GridCdt is None:
                    m_GridCdt=[iX,jY]
                else:
                    m_GridCdt=np.vstack((m_GridCdt, [iX,jY]))

        r_GridCdt, c_GridCdt=m_GridCdt.shape
        r_BSCdt, c_BSCdt=m_BSCdt.shape
        m_DisGrid2BS=np.zeros((r_GridCdt,r_BSCdt))
        m_virtlBSCdt=m_BSCdt
        m_virtlBSCdt[:,1]=m_virtlBSCdt[:,1]+50
        m_H_Azimuth=np.zeros((r_GridCdt,r_BSCdt))
        m_V_Azimuth=np.zeros((r_GridCdt,r_BSCdt))
        m_AntAttenuate=np.zeros((r_GridCdt,r_BSCdt))
        mm_AntAttenuate = sys.getsizeof(m_AntAttenuate)/1e6
        for iG in range(r_GridCdt):
            if 0==(np.abs(iG)%2000):
                print('%.0fs: %s [%s, iG Prog] = [%.f/%.f, %.3f, %.2fMB]' % ((time.time()-self.__t_start), sys._getframe().f_lineno, sys._getframe(0).f_code.co_name, iG/1000, r_GridCdt/1000, iG/r_GridCdt, mm_AntAttenuate))
            for iBS in range(r_BSCdt):
                m_DisGrid2BS[iG, iBS]=np.sqrt((m_GridCdt[iG,0]-m_BSCdt[iBS,0])**2+(m_GridCdt[iG,1]-m_BSCdt[iBS,1])**2)

                t_virtlDisGrid2BS=np.sqrt((m_GridCdt[iG,0]-m_virtlBSCdt[iBS,0])**2+(m_GridCdt[iG,1]-m_virtlBSCdt[iBS,1])**2)
                t_valCos=np.min([(50**2 + m_DisGrid2BS[iG, iBS]**2 - t_virtlDisGrid2BS**2) / (2 * 50 * m_DisGrid2BS[iG, iBS]+.0001),1])
                t_H_azimuth=round(180 * np.arccos(t_valCos) / np.pi)
                if m_GridCdt[iG,0]<m_BSCdt[iBS,1]:
                    t_H_azimuth += 180
                t_H_azimuth %= 360
                m_H_Azimuth[iG,iBS]=t_H_azimuth

                t_V_azimuth=round(180 * np.arctan(m_DisGrid2BS[iG, iBS] / (self.__h_BS - self.__h_UT)) / np.pi)
                t_V_azimuth = (t_V_azimuth + 180)%180
                m_V_Azimuth[iG, iBS] = t_V_azimuth
                t_H_azimuth %= 120
                if t_H_azimuth >= 60:
                    t_H_azimuth -=60
                m_AntAttenuate[iG, iBS] = np.copy(m_AntPatten[t_H_azimuth, t_V_azimuth])

        strIndex=['Grid_'+str(i+1) for i in range(m_DisGrid2BS.shape[0])]
        strColum=['Cell_'+str(i+1) for i in range(m_DisGrid2BS.shape[1])]
        n_m_RSRP = m_DisGrid2BS.shape[0]
        t_idx = int(n_m_RSRP/self.__n_DF)
        for iD in range(self.__n_DF):
            if iD<(self.__n_DF-1):
                mt_RSRP = np.round(m_DisGrid2BS[iD*t_idx:(iD+1)*t_idx,:], 3)
                t_strIndex = strIndex[iD*t_idx:(iD+1)*t_idx]
            else:
                mt_RSRP = np.round(m_DisGrid2BS[(iD)*t_idx:,:], 3)
                t_strIndex = strIndex[(iD)*t_idx:]
            pd_DF=pd.DataFrame(data=mt_RSRP, index=t_strIndex, columns=strColum)
            pd_DF.to_csv((self.__docHeader+'2DisGrid2BS'+str(iD)+'.csv').replace('p/CSV_','p/net/CSV_'),index=True)
        del m_DisGrid2BS, m_BSCtnCdt, m_AntPatten, m_H_Azimuth, m_V_Azimuth
        del pd_DF
        gc.collect()
        n_m_RSRP = m_AntAttenuate.shape[0]
        t_idx = int(n_m_RSRP/self.__n_DF)
        for iD in range(self.__n_DF):
            if iD<(self.__n_DF-1):
                mt_RSRP = np.round(m_AntAttenuate[iD*t_idx:(iD+1)*t_idx,:], 3)
                t_strIndex = strIndex[iD*t_idx:(iD+1)*t_idx]
            else:
                mt_RSRP = np.round(m_AntAttenuate[(iD)*t_idx:,:], 3)
                t_strIndex = strIndex[(iD)*t_idx:]
            pd_DF=pd.DataFrame(data=mt_RSRP, index=t_strIndex, columns=strColum)
            pd_DF.to_csv((self.__docHeader+'2AntAttuate'+str(iD)+'.csv').replace('p/CSV_','p/net/CSV_'),index=True)
